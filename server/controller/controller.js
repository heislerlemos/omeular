	...
	casa.save(casa)
		...
		.catch((err) => {
			res.status(500).send({
				message:
					err.message ||
					"Algum erro ocorreu enquanto criavamos o casa",
			});
		});
};
		...
		Casadb.findById(id)
			.then((data) => {
				if (!data) {
					res.status(404).send({
						message:
							"Não foi encontrado o utilizador pelo id " +
							id,
					});
				} else {
					res.send(data);
			...
			.catch((err) => {
				res.status(500).send({
					message: "Erro a obter a casa" + id,
				});
			});
	} else {
		Casadb.find()
			...
			.catch((err) => {
				res.status(500).send({
					message:
						err.message ||
						"Um erro occoreu enquanto  buscava-se uma casa",
				});
			});
	}
};
...
exports.delete = (req, res) => {
	...
	Casadb.findByIdAndDelete(id)
		...
		.then((data) => {
			if (!data) {
				res.status(404).send({
					message: `Não é possivel apagar o id ${id}, talvez o id esteja errado`,
				});
			} else {
				res.send({
		...
		.catch((err) => {
			res.status(500).send({
				message:
					"Não foi possivel apagar utilizador com o id " +
					id,
			});
		});
};
